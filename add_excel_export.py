"""
CIPHER Excel Export
Add this to routes.py by running: python add_excel_export.py
"""
...
if '/export/excel' not in c:
    c += route
    shutil.copy(ROUTES, ROUTES + '.excel.bak')
    with open(ROUTES, 'w', encoding='utf-8') as f:
        f.write(c)
    print("Added Excel export route to routes.py")
else:
...
old_topbar = '      <div class="topbar-badge" id="dir-period-badge">Q2 2026</div>'
new_topbar = '''      <div class="topbar-badge" id="dir-period-badge">Q2 2026</div>
      <button onclick="exportExcel()" style="padding:8px 18px;background:#2e7d32;color:#fff;border:none;border-radius:8px;font-size:10px;letter-spacing:2px;font-family:'Courier New',monospace;cursor:pointer;transition:background 0.2s;" onmouseover="this.style.background='#1b5e20'" onmouseout="this.style.background='#2e7d32'">⬇ EXPORT EXCEL</button>'''

if 'exportExcel' not in h:
    if old_topbar in h:
        h = h.replace(old_topbar, new_topbar, 1)
        print("Added Export button to director topbar")
    else:
        print("WARNING: Could not find topbar badge to insert button")
  ...
  function exportExcel() {
    ...
    fetch(`${API}/export/excel`)
      ...
      .then(blob => {
        ...
        a.href = url;
        a.download = `CIPHER_Export_${new Date().toISOString().slice(0,10)}.xlsx`;
        a.click();
    ...
    last = h.rfind('</script>')
    h = h[:last] + export_js + '\n' + h[last:]
    print("Added exportExcel JS function")
else:
    print("Export button already present")

with open(HTML, 'w', encoding='utf-8') as f:
    f.write(h)

print("\nDone!")
print("  pip install openpyxl  (if not already installed)")
print("  Restart: uvicorn main:app --reload")
print("  Log in as director — green EXPORT EXCEL button appears in topbar")
